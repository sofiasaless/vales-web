...
const PaymentConfirmationScreen = () => {
  ...
  const labelSalary =
    employee.tipo === "FIXO" ? "Valor da quinzena" : "Total das diárias";
  const labelBaseSalary =
    employee.tipo === "FIXO"
      ? `Salário base R$ ${employee.salario.toFixed(2)}`
      : `Diária R$ ${employee.salario.toFixed(2)}`;
  ...
  const renderVoucherItem = (item: Vale) => (
    <div
      ...
      style={{
        ...
        padding: "8px 12px",
        borderRadius: 8,
        backgroundColor: theme.colors.colorErrorTransparent,
      }}
    >
      <div>
      ...
      >
        ...
        <Card className="glass-card">
          ...
          >
            ...
            <span style={{ color: "var(--text-secondary)", fontWeight: 500 }}>
              Total do Vale a Descontar
            </span>
          </div>
          <MoneyDisplay
            value={-calculateTotalVauchers(employee.vales)}
            size="xl"
            variant="negative"
          />
        ...
        <Card style={{ background: "rgba(34,197,94,0.1)" }}>
          ...
          >
            ...
            <span
              style={{ color: "var(--success)", fontWeight: 600, fontSize: 18 }}
            >
              Total a Pagar
            </span>
          </div>
          <MoneyDisplay
            value={calculateAmount(employee)}
            size="xl"
            variant={calculateAmount(employee) < 0 ? "negative" : "positive"}
          />
        </Card>
