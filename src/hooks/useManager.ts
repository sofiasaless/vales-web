...
import {
  GerenteAutenticatedResponseBody,
  GerenteAutenticateRequestBody,
  GerentePostRequestBody,
  GerenteResponseBody,
  GerenteUpdateRequestBody,
} from "@/types/gerente.type";
...
export function useListManagers() {
  return useQuery({
    ...
    gcTime: 0, // não guardar cache entre sessões
    refetchOnMount: true,
...
export function useManagers() {
  ...
  const autenticate = useMutation({
    mutationFn: ({ body }: { body: GerenteAutenticateRequestBody }) =>
      ManagerService.autenticate(body),

    onSuccess: (value: GerenteAutenticatedResponseBody) => {
      if (value.usuario)
        localStorage.setItem("usuario", JSON.stringify(value.usuario));
    },
  ...
  const create = useMutation({
    mutationFn: ({ body }: { body: GerentePostRequestBody }) =>
      ManagerService.create(body),
  ...
  const update = useMutation({
    mutationFn: ({
      ...
      managerId: string;
      payload: GerenteUpdateRequestBody;
    }) => ManagerService.update(managerId, payload),
